...
if (!empty($arParams["SIMILAR_ITEMS"]) && is_array($arParams["SIMILAR_ITEMS"])) {
    $arResult["ITEMS"] = array_filter($arResult["ITEMS"], function($item) use ($arParams) {
        return in_array($item["ID"], $arParams["SIMILAR_ITEMS"]);
    });
}
...
if ($arParams["SEARCH_PAGE"])
{
    ...
    {
        echo '<script>console.log(' . json_encode($arItem) . ');</script>';
